...
function ResourceCard({ resource }) {
  ...
  return (
    <div
      className="glass rounded-2xl border border-white/10 hover:border-white/20 p-6
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-4">
        <div
          className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${color.bg} border ${color.border}`}
        >
          <Icon size={20} className={color.text} />
        </div>

        <div className="flex-1 min-w-0">
          <span className={`text-xs font-mono ${color.text} block mb-1`}>
            {resource.category}
          </span>
      ...
      <div className="flex items-center justify-between border-t border-white/10 pt-4">
        ...
        {resource.url ? (
          ...
          <button
            onClick={() =>
              alert("Download coming soon — resources are being finalized!")
            }
            className="flex items-center gap-1.5 text-xs font-display tracking-wide px-3 py-1.5
                       rounded-lg bg-white/5 border border-white/10 text-white/60
                       hover:border-cyan-400/40 hover:text-cyan-400 transition-all duration-200"
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mt-16 glass p-8 rounded-2xl border border-violet-400/20 text-center">
          ...
          <p className="text-white/60 mb-6">
            Request a custom lesson plan or activity sheet tailored to your curriculum.
          </p>
          <a
            href="mailto:aionthestreets@example.com"
